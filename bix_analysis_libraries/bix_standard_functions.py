...
def interpolate(df, step, kind="linear"):
    ...
    int_df = []
    for _, data in df.T.groupby(df.columns):
        y = data.values[0]
        f = interp1d(x, y, kind)
        int_data = pd.DataFrame(f(xnew), index=xnew, columns=data.index)
...
def apply_savgol(df, window_length=25, polyorder=3, deriv=0, **kwargs):
    ...
    def savgol(x):
        return savgol_filter(
            x, window_length=window_length, polyorder=polyorder, deriv=deriv, **kwargs
        )
...
def unique_legend(ax):
    ...
    by_label = {"handles": by_label.values(), "labels": by_label.keys()}
    return by_label
